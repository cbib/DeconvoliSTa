...
The pipeline runs on a normal Linux workstation for the test data, and scales to an HPC/GPU
cluster for real datasets. GPU methods (cell2location, DDLS) need an NVIDIA GPU whose driver
matches the container's CUDA build (the cell2location image ships **CUDA 12.8**, for 570-series
drivers; a 24 GB card or an H100 MIG slice is enough for the demo). See
[docs/data_and_resources.md](/docs/data_and_resources.md) for per-method CPU/RAM/runtime estimates.

Large single-cell references (≈3 GB+) can exhaust RAM: reduce the file (drop unused
`annotation_level`s / assays) or subsample cells per type before running.
...
| `seurat_res` | `"0.5"` | Seurat clustering resolution (higher → more clusters). |
...
```bash
snakemake -s main.smk -c8 --use-singularity --singularity-args '--nv' \
    --config mode="run_dataset" methods="cell2location,rctd" \
    sc_input="unit-test/test_sc_data.rds" sp_input="unit-test/test_sp_data.rds" \
    output="res" annot="subclass" use_gpu="true" map_genes="false" skip_metrics="true"
```

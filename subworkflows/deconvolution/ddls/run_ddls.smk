...
map_genes = config["map_genes"] if "map_genes" in config.keys() else "false"
...
rule run_ddls:
    ...
    output:
        output
    singularity:
        # cbib clean/minimal SpatialDDLS image (Dockerfile.cbib) on GHCR; replaces the large
        # unoptimised docker://abderahim02/sp_ddls.
        "docker://ghcr.io/cbib/sp_ddls:latest"
    threads:
        12
